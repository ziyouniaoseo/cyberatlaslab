...
/**
 * ============================================================================================================================
 * 11.5 货币符号规范化辅助：USD/EUR/GBP 等代码自动转符号，已是符号则原样放行
 * ============================================================================================================================
 */
if (!function_exists('crs_format_currency')) {
    function crs_format_currency($raw)
    {
        $raw = trim((string) $raw);
        if ($raw === '')
            return '$';
        $map = ['USD' => '$', 'EUR' => '€', 'GBP' => '£', 'JPY' => '¥', 'CAD' => 'C$', 'AUD' => 'A$'];
        return isset($map[strtoupper($raw)]) ? $map[strtoupper($raw)] : $raw;
    }
}
...
if (!function_exists('crs_sidebar_toc_shortcode')) {
    function crs_sidebar_toc_shortcode($atts = [])
    {
        ...
        // 3. Transient 高性能缓存检索
        $cache_key = 'crs_toc_v4_b_' . $post_id; // 升级版本号后缀，防止主题更新时旧缓存数据格式冲突
        $toc_items = get_transient($cache_key);

        // 4. 缓存穿透：当没有缓存或缓存过期时，启动正则提取引擎
        if ($toc_items === false || empty($toc_items)) {
            $toc_items = crs_extract_toc_items($post_id);

            if (!empty($toc_items)) {
                // 有效数据成功存入数据库 Transient 对象缓存，有效期为 24 小时
                set_transient($cache_key, $toc_items, DAY_IN_SECONDS);
            } else {
                // 性能防抖：如果没有提取到目录项，写一个 5 分钟的空数组短期缓存，防止被恶意扫描导致数据库瞬间过载
    ...
    function crs_extract_toc_items($post_id)
    {
        ...
        if (preg_match_all($pattern, $content, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                $toc_items[] = [
                    'anchor' => sanitize_title($anchor), // 清洗为标准的标准 URL 锚点格式
                    'label' => sanitize_text_field($label), // 清洗文本字段
                ];
            }
        }
...
if (!function_exists('crs_extract_html_from_elementor')) {
    function crs_extract_html_from_elementor($elements)
    {
        ...
        foreach ($elements as $element) {
            ...
            // 途径 B：抓取 Elementor 官方原生 Heading 标题组件，并就地将其格化转换为带有 data-toc 的 H2 标准语义标签
            if (isset($element['widgetType']) && $element['widgetType'] === 'heading') {
                if (isset($element['settings']['title'])) {
                    $html .= '<h2 id="' . (isset($element['id']) ? $element['id'] : '') . '" ';
                    // 透传用户在 Elementor 高级设置里手动附加的自定义 HTML 属性
                    if (isset($element['settings']['_attributes'])) {
                        $html .= $element['settings']['_attributes'] . ' ';
                    }
                    $html .= 'data-toc="' . esc_attr($element['settings']['title']) . '">';
...
if (!function_exists('crs_clear_toc_cache')) {
    function crs_clear_toc_cache($post_id)
    {
        ...
        delete_transient('crs_toc_v4_b_' . $post_id);
    }
}
...
if (!function_exists('crs_toc_mobile_shortcode')) {
    function crs_toc_mobile_shortcode($atts) {
        ...
        // 3. 多端共用对象缓存检索
        $cache_key = 'crs_toc_v4_b_' . $post_id;
        $toc_items = get_transient($cache_key);
...
function crs_enqueue_toc_assets() {
    if (is_singular('product')) {
        wp_enqueue_script(
            'crs-toc-highlight',
            get_stylesheet_directory_uri() . '/js/toc-highlight.js',
            [],
            '1.0.0',
            true
        );
