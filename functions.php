...
/**
 * =============================================================================
 * 39. 分类页锚点导航 - [cal_anchor_nav]（v3 · 定稿版）
 * =============================================================================
 * 结构：.cal-anchor-nav-wrap > sentinel + 桌面 pill bar + 移动 Contents 面板
 * 数据：同一份 $items 数组双容器渲染，count 动态
 * 图标：cal_get_icon('toc_icon' / 'chevron_down_icon')，内联 SVG 兜底
 * JS：cal-anchor-scrollspy.js 唯一版本，随短代码输出、全页只打印一次
 * ⚠️ 不要使用 cal-anchor-accordion.js（与 scrollspy 重复绑定 toggle 会互相抵消）
 * =============================================================================
 */
if (!function_exists('cal_anchor_nav_shortcode')) {
    ...
    {
        $items = array(
            array('id' => 'comparison',  'label' => __('Comparison', 'cyberatlaslab')),
            array('id' => 'methodology', 'label' => __('How We Test', 'cyberatlaslab')),
            array('id' => 'picks',       'label' => __('Top Picks', 'cyberatlaslab')),
            array('id' => 'guide',       'label' => __('Buying Guide', 'cyberatlaslab')),
            array('id' => 'faq',         'label' => __('FAQ', 'cyberatlaslab')),
        );
        $count = count($items);

        $toc_icon = function_exists('cal_get_icon') ? cal_get_icon('toc_icon') : '';
        if ($toc_icon === '') {
            $toc_icon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="12" x2="14" y2="12"/><line x1="4" y1="18" x2="17" y2="18"/></svg>';
        }
        $chevron_icon = function_exists('cal_get_icon') ? cal_get_icon('chevron_down_icon') : '';
        if ($chevron_icon === '') {
            $chevron_icon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>';
        }

        ob_start();
        ?>
        <div class="wrap cal-anchor-nav-wrap">
            <span class="sentinel cal-anchor-nav__sentinel" aria-hidden="true"></span>

            <!-- 桌面端：sticky pill bar（≥1025px） -->
            <nav class="cal-anchor-nav" aria-label="<?php esc_attr_e('Page sections', 'cyberatlaslab'); ?>">
                <div class="nav-inner cal-anchor-nav__inner">
                    <?php foreach ($items as $i => $item) : ?>
                        <a href="#<?php echo esc_attr($item['id']); ?>" data-anchor="<?php echo esc_attr($item['id']); ?>">
        ...
        // scrollspy JS：唯一版本，全页只输出一次（元素在其之前已渲染，IIFE 直接执行即可）
        ...
        <script>
        (function () {
            var deskNav = document.querySelector('.cal-anchor-nav');
            var sentinel = document.querySelector('.cal-anchor-nav__sentinel');
            var links = Array.prototype.slice.call(document.querySelectorAll('.cal-anchor-nav__inner a[data-anchor], .cal-anchor-nav-mobile__list a[data-anchor]'));
            var hasDesk = !!(deskNav && sentinel && links.length);

            var mobile = document.querySelector('.cal-anchor-nav-mobile');
            var mToggle = mobile ? mobile.querySelector('.cal-anchor-nav-mobile__toggle') : null;
            var mList = mobile ? mobile.querySelector('.cal-anchor-nav-mobile__list') : null;

            if (hasDesk && (!mToggle || !mList)) return;

            /* —— 桌面吸顶双态 —— */
            if (hasDesk) {
                if ('IntersectionObserver' in window) {
                    new IntersectionObserver(function (entries) {
                        deskNav.classList.toggle('is-stuck', !entries[0].isIntersecting);
                    }, { threshold: 0 }).observe(sentinel);
                } else {
                    var stuckFallback = function () {
                        var top = sentinel.getBoundingClientRect().top;
                        deskNav.classList.toggle('is-stuck', top <= 0);
                    };
                    window.addEventListener('scroll', stuckFallback, { passive: true });
                    stuckFallback();
                }
            }
            ...
            /* —— scrollspy：一处计算，双端同步 —— */
            var offset = 140;
            var idLinks = links.filter(function (l) { return l.closest('.cal-anchor-nav__inner'); });
            if (!idLinks.length) idLinks = links;
            ...
            var onScroll = function () {
                ...
                idLinks.forEach(function (l) {
                    var id = l.getAttribute('data-anchor');
                ...
                if (!current && firstId) current = firstId;
            ...
            /* —— 移动端展开/收起（唯一绑定，禁止再挂 accordion.js） —— */
            ...
            var toggleMobile = function () {
                setMobile(mToggle.getAttribute('aria-expanded') !== 'true');
            };

            if (mToggle) mToggle.addEventListener('click', toggleMobile);
