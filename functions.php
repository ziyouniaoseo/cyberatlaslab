...
if (!function_exists('cal_get_icon')) {
    ...
    function cal_get_icon($key, $class = '')
    {
        ...
        // 各模块向右折叠箭头图标
        'right_collapse_arrow_icon' => '<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m9 18 6-6-6-6"/></svg>',
        ];
...
/**
 * =============================================================================
 * 21. 亮点总结模块简码 - Key Takeaways (带全新无障碍骨架、动态副标题与动态中继器)
 * 简码：[cal_takeaways]
 * =============================================================================
 * 优化说明：
 * 1. 严格手动输入控制：优先读取后台文本字段 takeaway_title。若不填，前台彻底熔断。
 * 2. 动态副标题增强：支持平级字段 takeaway_subtitle，填则输出，不填不占用 DOM 空间。
 * 3. 智能防空壳门禁：即使填了标题，但若底下的中继器列表未添加任何有效行，同样强行拦截渲染。
 * 4. 语义化与 A11y 封顶：升级为 section[aria-labelledby] 架构，引入独立的 header 隔离层。
 */
...
if (!shortcode_exists('cal_takeaways')) {
    add_shortcode('cal_takeaways', function ($atts) {
        ...
        // 2. 严格控制：从外层独立字段中读取主标题与副标题（平级调用，不进循环）
        $takeaway_title    = trim((string) get_field('takeaway_title', $post_id));
        $takeaway_subtitle = trim((string) get_field('takeaway_subtitle', $post_id));
        ...
        // 动态副标题：仅在后台字段非空时才渲染 DOM
        if ($takeaway_subtitle !== '') {
            $output .= '<p class="crs-takeaways__subtitle">' . esc_html($takeaway_subtitle) . '</p>';
        }
        ...
        foreach ($clean_items as $item) {
            ...
            // 图标外包裹 span，并在 SVG 生成函数内部对读屏器隐藏（aria-hidden），规避视觉混淆
 ...
 *       新增：scoring_subtitle 副标题支持，统一使用 header 标签包裹标题组
 * =============================================================================
 */
if (!shortcode_exists('cal_scores')) {
    ...
    add_shortcode('cal_scores', function ($atts) {
        ...
        // 获取副标题字段
        $scoring_subtitle = trim((string) get_field('scoring_subtitle', $post_id));
        ...
        <div class="crs-scores__inner">
            <div class="crs-scores__box">
                ...
                <header class="crs-scores__header">
                    ...
                    <?php if ($scoring_subtitle !== ''): ?>
                        <p class="crs-scores__subtitle"><?php echo esc_html($scoring_subtitle); ?></p>
                    <?php endif; ?>
                </header>
...
/**
 * =============================================================================
 * 23. FAQ模块简码 - [cal_faq]（v2.3 · Header及副标题版）
 * =============================================================================
 * 新增：① show_title 控制 header 标题块渲染
 *       ② 新增 faq_subtitle / {$option_key}_subtitle 副标题获取与渲染
 *       ③ 标题与副标题使用 <header> 标签统包裹
...
if (!shortcode_exists('cal_faq')) {
    add_shortcode('cal_faq', function ($atts) {
        ...
        if ($atts['source'] === 'option') {
            ...
            $faq_subtitle = trim((string) get_field($atts['option_key'] . '_subtitle', 'option'));
            ...
            $faq_subtitle = trim((string) get_field('faq_subtitle', $post_id));
            ...
            <div class="crs-faq__box">
                ...
                <?php if ($show_title && $faq_title !== ''): ?>
                    <header class="crs-faq__header">
                        ...
                        <?php if ($faq_subtitle !== ''): ?>
                            <p class="crs-faq__subtitle"><?php echo esc_html($faq_subtitle); ?></p>
                        <?php endif; ?>
                    </header>
                <?php endif; ?>
 ...
 * 动态字段：technical_title (主标题), technical_subtitle (副标题)
 * =============================================================================
 */
...
if (!function_exists('crs_tech_snapshot_shortcode')) {
    ...
    function crs_tech_snapshot_shortcode($atts = [])
    {
        ...
        // 动态获取标题与副标题，带优雅兜底
        $tech_title_raw = trim((string) get_field('technical_title', $post_id));
        $tech_subtitle_raw = trim((string) get_field('technical_subtitle', $post_id));

        $tech_title = $tech_title_raw !== '' ? $tech_title_raw : __('Technical Snapshot', 'cyberatlas-core');
        $tech_subtitle = $tech_subtitle_raw !== '' ? $tech_subtitle_raw : __('Key technical specifications.', 'cyberatlas-core');
        ...
        <section class="crs-snapshot" aria-labelledby="crs-snapshot-title" data-product-id="<?php echo esc_attr($post_id); ?>"
            ...
            <header class="crs-snapshot__header">
                <h2 id="crs-snapshot-title" class="crs-snapshot__title">
                    ...
                    <?php endif; ?>
                    <span><?php echo esc_html($tech_title); ?></span>
                </h2>
                <?php if (!empty($tech_subtitle)): ?>
...
/**
 * =============================================================================
 * 36. 分类页快速对比表 - [cal_compare_table]（v4 · Desktop Table + Mobile Compact Snapshot）
 * =============================================================================
 * 数据源：当前页 cat_picks_products relationship（顺序即排名 #1~#5），单次取值，
 *   桌面 Table 与移动 Snapshot 两套 markup 共用同一份 $rows，不重复查询 ACF
 * 结构：桌面真 <table>（语义完整，不再用 CSS 把 tr/td 强行伪装成卡片）；
 *   移动端独立 Compact Snapshot（Grid 布局，非表格语义，避免破坏 accessibility tree）
 * Score Engine：桌面表头 + 移动端逐行均挂载 Overall 维度 tooltip
 *   （crs_render_score_tooltip('overall')），直接复用测评页已实现的弹层交互
 * 新增字段：product_tagline（产品名称下方一行简短副标题，与 Best For 语义不同，
 *   为空则不渲染该行）
 * 信息层级：Rank(+Top Pick) │ Product(+Tagline) │ Score(+Tooltip) │
 *   Price(+Discount) │ Best For │ CTA(+Read Review)
 * 空值纪律：无关联产品整块不渲染；行内缺数据只缺对应单元格/子项；
 *   无有效链接不渲染 CTA；无有效详情页链接不渲染 Read Review
 * 用法：[cal_compare_table]（可选属性 title / intro，均有默认文案）
...
if (!function_exists('cal_compare_table_shortcode')) {
    ...
    function cal_compare_table_shortcode($atts)
    {
        ...
        $products = get_field('cat_picks_products', $post_id);
        ...
        // 桌面 Table 和移动 Snapshot 两套 markup 都从同一份 $rows 渲染
        // ---------------------------------------------------------------
        $rows = array();
        $rank = 0;
        ...
        foreach ($products as $product) {
            ...
            $tagline = get_field('product_tagline', $pid);
            ...
            // Read Review：站内详情页内链，只有已发布且有真实 permalink 时才渲染
            $review_url = get_permalink($pid);
            $has_review = (!empty($review_url) && get_post_status($pid) === 'publish');
        ...
        <section class="cal-compare" id="comparison" aria-labelledby="cal-compare-title-<?php echo esc_attr($post_id); ?>">
            ...
            <p class="cal-compare__intro"><?php echo esc_html($intro); ?></p>

            <!-- ============ Desktop：原生 <table>，语义完整 ============ -->
            <div class="cal-compare__desktop">
                    ...
                    <table class="cal-compare__table">
                        ...
                        <tbody>
                            <?php foreach ($rows as $row) : ?>
                                <tr class="cal-compare__row<?php echo $row['is_top'] ? ' cal-compare__row--top' : ''; ?>">
                                    <td class="cal-compare__cell-rank">
                                        <span class="cal-compare__rank-stack">
                                            <span class="cal-compare__rank-badge"><?php echo esc_html('#' . $row['rank']); ?></span>
                                            <?php if ($row['is_top']) : ?>
                                                <span class="cal-compare__top-pick"><?php esc_html_e('Top Pick', 'cyberatlaslab'); ?></span>
                                            <?php endif; ?>
                                        </span>
                                    </td>
                                    <td class="cal-compare__cell-product">
                                        <span class="cal-compare__logo"><?php echo $row['logo']; ?></span>
                                        <span class="cal-compare__name-col">
                                            <span class="cal-compare__name"><?php echo esc_html($row['name']); ?></span>
                                            <?php if ($row['tagline'] !== '') : ?>
                                                <span class="cal-compare__tagline"><?php echo esc_html($row['tagline']); ?></span>
                                            <?php endif; ?>
                                        </span>
                                    </td>
                                    <td class="cal-compare__cell-score">
                                        <?php if ($row['score'] !== '') : ?>
                                            <span class="cal-compare__score"><?php echo esc_html($row['score']); ?><span class="cal-compare__score-max">/10</span></span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="cal-compare__cell-price">
                                        <?php if ($row['price'] !== '') : ?>
                                            <span class="cal-compare__price"><?php echo esc_html($row['price']); ?><?php echo $row['period'] !== '' ? '<span class="cal-compare__period">' . esc_html($row['period']) . '</span>' : ''; ?></span>
                                        <?php endif; ?>
                                        <span class="cal-compare__discount-slot">
                                            <?php if ($row['discount'] !== '') : ?>
                                                <span class="cal-compare__discount"><?php echo esc_html($row['discount']); ?></span>
                                            <?php endif; ?>
                                        </span>
                                    </td>
                                    <td class="cal-compare__cell-bestfor">
                                        <?php echo $row['best_for'] !== '' ? esc_html($row['best_for']) : ''; ?>
                                    </td>
                                    <td class="cal-compare__cell-cta">
                                        <span class="cal-compare__cta-stack">
                                            <?php if ($row['has_cta'] && function_exists('cal_render_link_button')) : ?>
                                                <?php
            ...
            <div class="cal-compare__mobile">
                <div class="cal-compare__snapshot-list">
                    <?php foreach ($rows as $row) : ?>
                        <article class="cal-snapshot-card<?php echo $row['is_top'] ? ' cal-snapshot-card--top' : ''; ?>">
                            <div class="cal-snapshot-card__identity">
                                <div class="cal-snapshot-card__who">
                                    <span class="cal-snapshot-card__logo"><?php echo $row['logo']; ?></span>
                                    <div class="cal-snapshot-card__name-col">
                                        <div class="cal-snapshot-card__name-row">
                                            <span class="cal-compare__rank-badge"><?php echo esc_html('#' . $row['rank']); ?></span>
                                            <span class="cal-snapshot-card__name"><?php echo esc_html($row['name']); ?></span>
                                            <?php if ($row['is_top']) : ?>
                                                <span class="cal-snapshot-card__top-pick"><?php esc_html_e('Top Pick', 'cyberatlaslab'); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <?php if ($row['tagline'] !== '') : ?>
                                            <span class="cal-snapshot-card__tagline"><?php echo esc_html($row['tagline']); ?></span>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="cal-snapshot-card__score">
                                    <?php if ($row['score'] !== '') : ?>
                            ...
                            <div class="cal-snapshot-card__price-row">
                                <span>
                                    <?php if ($row['price'] !== '') : ?>
                                        <span class="screen-reader-text"><?php esc_html_e('Price:', 'cyberatlaslab'); ?></span>
                                        <span class="cal-snapshot-card__price"><?php echo esc_html($row['price']); ?><?php echo $row['period'] !== '' ? '<span class="cal-snapshot-card__period">' . esc_html($row['period']) . '</span>' : ''; ?></span>
                                    <?php endif; ?>
                                </span>
                                <?php if ($row['discount'] !== '') : ?>
                                    <span class="cal-snapshot-card__discount"><?php echo esc_html($row['discount']); ?></span>
                                <?php else : ?>
                                    <span class="cal-snapshot-card__discount-placeholder"></span>
                                <?php endif; ?>
                            </div>

                            <?php if ($row['best_for'] !== '') : ?>
                                <div class="cal-snapshot-card__bestfor" title="<?php echo esc_attr($row['best_for']); ?>">
                                    <span class="screen-reader-text"><?php esc_html_e('Best for:', 'cyberatlaslab'); ?></span>
                                    <?php esc_html_e('Best for', 'cyberatlaslab'); ?> &middot; <strong><?php echo esc_html($row['best_for']); ?></strong>
                                </div>
                            <?php endif; ?>

                            <div class="cal-snapshot-card__cta-row">
                                <?php if ($row['has_cta'] && function_exists('cal_render_link_button')) : ?>
                                    <?php
                                    echo cal_render_link_button(array(
                                        'url'   => $row['cta_url'],
 ...
 * =============================================================================
 * 数据源：cat_picks_products（与 [cal_compare_table] 同源，顺序即排名）
...
/**
 * =============================================================================
 * 40. 分类页锚点导航 - [cal_anchor_nav]（v3.2 · 终版）
 * =============================================================================
 * 结构：.cal-anchor-nav-wrap > sentinel + 桌面 pill bar + 移动 Contents 面板
 * 条目：6 项（comparison/methodology/picks/guide/factors/faq），与页面结构顺序一致
 * 图标：cal_get_icon 映射库（toc_icon 子弹头 / chevron_down 倒三角），均已入库无兜底
 * JS：外置 /js/cal-anchor-nav.js（defer enqueue，见 cal_enqueue_anchor_nav_assets）
 * =============================================================================
 */
if (!function_exists('cal_anchor_nav_shortcode')) {
    ...
    function cal_anchor_nav_shortcode($atts)
    {
        ...
        // 图标：映射库直取（toc_icon 子弹头 / chevron_down 倒三角）
        $toc_icon     = function_exists('cal_get_icon') ? cal_get_icon('toc_icon') : '';
        $chevron_icon = function_exists('cal_get_icon') ? cal_get_icon('chevron_down') : '';

        ob_start();
        ?>
        <div class="cal-anchor-nav-wrap">
            <span class="cal-anchor-nav__sentinel" aria-hidden="true"></span>

            <!-- 桌面端：sticky pill bar（≥1025px） -->
            <nav class="cal-anchor-nav" aria-label="<?php esc_attr_e('Page sections', 'cyberatlaslab'); ?>">
                <div class="cal-anchor-nav__inner">
                    <?php $i = 0;
        foreach ($items as $id => $label) : $i++; ?>
                        <a href="#<?php echo esc_attr($id); ?>" data-anchor="<?php echo esc_attr($id); ?>">
            ...
            <!-- 移动端：Contents 折叠面板（≤1024px，对齐测评页） -->
            <div class="cal-anchor-nav-mobile">
                <button type="button" class="cal-anchor-nav-mobile__toggle" aria-expanded="false" aria-controls="cal-anchor-nav-mobile-list">
                    <span class="cal-anchor-nav-mobile__toggle-left">
                        <span class="cal-anchor-nav-mobile__toc-icon" aria-hidden="true"><?php echo $toc_icon; ?></span>
                        <span class="cal-anchor-nav-mobile__title"><?php esc_html_e('Contents', 'cyberatlaslab'); ?></span>
                    </span>
                    <span class="cal-anchor-nav-mobile__toggle-right">
                        <span class="cal-anchor-nav-mobile__count"><?php printf(esc_html__('%d sections', 'cyberatlaslab'), (int) $count); ?></span>
                        <span class="cal-anchor-nav-mobile__chevron" aria-hidden="true"><?php echo $chevron_icon; ?></span>
                    </span>
                </button>
                <ol class="cal-anchor-nav-mobile__list" id="cal-anchor-nav-mobile-list" hidden>
                    <?php $i = 0;
        foreach ($items as $id => $label) : $i++; ?>
                        <li>
                            <a href="#<?php echo esc_attr($id); ?>" data-anchor="<?php echo esc_attr($id); ?>">
 ...
 * 解析 cat_picks_products 第 1 名 → 委托 §18 移动卡渲染（样式/显隐沿用其 CSS）
 ...
 * （不再单一依赖 cat_picks_products，字段组扩充时往数组里加即可）
 */
if (!function_exists('cal_is_category_page')) {
    function cal_is_category_page($post_id)
    {
        ...
        $markers = array('cat_picks_products', 'cat_hero_intro', 'cat_how_test_content');
