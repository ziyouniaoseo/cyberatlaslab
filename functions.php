...
add_shortcode('acf_repeater', function($atts, $content = null) {
    ...
    if (empty($atts['field']) || empty($content)) return '';
...
add_shortcode('acf_list', function($atts) {
    ...
    if (empty($atts['field']) || empty($atts['sub'])) return '';
...
if (!function_exists('crs_sidebar_specs_shortcode')) {
    function crs_sidebar_specs_shortcode($atts = [])
    {
        ...
        // 2. 调用第10项公共函数，安全读取当前页面缓存的产品数据集
        $data = crs_get_product_data();
        ...
        <nav class="crs-toc-mobile crs-toc-mobile--fusion" data-context="mobile" aria-labelledby="<?php echo esc_attr($button_label_id); ?>">
            ...
            <ul class="crs-toc-mobile__list" id="<?php echo esc_attr($list_wrapper_id); ?>" aria-hidden="true">
                <?php foreach ($toc_items as $index => $item) :
                    // SEO 体验升级：在 PHP 端利用已有的格式化函数清洗标签内容，确保输出给 AI 和爬虫的目录文本是首字母大写的 Title Case
                    $clean_label = function_exists('cal_format_verdict_title')
                        ? cal_format_verdict_title($item['label'])
                        : $item['label'];
                ?>
                    <li>
...
/* =============================================================================
   30. FONT PRELOAD SYSTEM（全站字体资源预加载 - 可变合流版）
   ============================================================================= */
add_action('wp_head', function () {

    // 子主题 fonts 目录（动态获取，避免硬编码路径）
    $font_base = get_stylesheet_directory_uri() . '/fonts/';

    // 只预加载完美控盘的可变字体，杜绝多余的请求
    printf(
        "\n\t\n" .
        "\t<link rel=\"preload\" href=\"%1\$s\" as=\"font\" type=\"font/woff2\" crossorigin>\n",
        esc_url($font_base . 'InterVariable.woff2')
    );
...
class CAL_CRS_Config
{
    ...
    const POST_TYPE = 'product';                     // 🛠️ 补回缺失常量：核心产品 CPT 标识
    const DEFAULT_PREFIX = 'visit';                  // 黄金默认路由前缀
    const REDIRECT_CODE = 302;                       // 联盟跳转的标准 HTTP 临时重定向状态码
    const CACHE_TTL = 86400;                         // 持久化缓存生命周期 (24小时)
    const LOG_INVALID_LINKS = true;                  // 是否独立开启异常联盟链接日志审计开关

    // ACF 选项页与字段标识
    const OPTION_PAGE_READ = 'option';                    // get_field() 读取 Options 页时所需的单数标识
    const OPTION_PAGE_SAVE = 'options';                   // acf/save_post 回调拦截时所需的复数标识
    const FIELD_RAW_URL = 'affiliate_url';                // 隐藏的原始联盟长链接字段名
    const FIELD_PREFIX = 'affiliate_prefix';              // 单品自定义前缀字段名
    const OPTION_PREFIXES = 'affiliate_prefix_whitelist'; // 全局白名单集中维护字段名 (Textarea)
    const OPTION_GLOBAL = 'global_affiliate_prefix';      // 全局默认前缀字段名 (Text)

    // 缓存与统计配置
    const CACHE_KEY = 'cal_crs_active_prefixes_v9';  // 独立高版本缓存 Key
    const CACHE_GROUP = 'cal_crs';                   // 独立的内存对象缓存分组名称
    const CLICK_META = 'crs_total_clicks';           // 单品点击打点的 Meta Key
}
            ...
            if ($rank === 1) {
                ...
                if (!empty($pros) && is_array($pros)) {
                    ...
                    foreach (array_slice($pros, 0, 3) as $row) {
                        if (empty($row['pro_item'])) continue;
...
/**
 * =============================================================================
 * 38. 分类页 Hero - [cal_cat_hero]
 * =============================================================================
 * H1 取页面标题；Trust Signals = cat_hero_tested / cat_hero_hours / 页面更新时间
 * Quick Filters = cat_hero_filters repeater → 锚点跳转片
 * 空值纪律：Intro/Filters 缺省则不渲染对应区块；三项 Trust 全空则整行隐藏
 * 用法：[cal_cat_hero]
 * =============================================================================
 */
if (!function_exists('cal_cat_hero_shortcode')) {
    ...
    function cal_cat_hero_shortcode($atts)
    {
        ...
        $intro   = get_field('cat_hero_intro', $post_id);
        $intro   = is_string($intro) ? trim($intro) : '';
        $tested  = get_field('cat_hero_tested', $post_id);
        $tested  = (is_numeric($tested) && $tested !== '') ? absint($tested) : 0;
        $hours   = get_field('cat_hero_hours', $post_id);
        $hours   = is_string($hours) ? trim($hours) : '';
        $updated = get_the_modified_date('M Y', $post_id);
        $filters = get_field('cat_hero_filters', $post_id);

        // Trust Signals 装配（空项自动跳过）
        $trust_items = array();
        if ($tested > 0) {
            $trust_items[] = array(
                'icon' => 'shield',
                'text' => sprintf(__('%s Products Tested', 'cyberatlaslab'), $tested),
            );
        }
        if ($hours !== '') {
            $trust_items[] = array('icon' => 'check', 'text' => $hours);
        }
        if ($updated !== '') {
            $trust_items[] = array(
                'icon' => 'calendar',
                'text' => sprintf(__('Updated: %s', 'cyberatlaslab'), $updated),
            );
        }
        ...
        <section class="cal-cat-hero">
            ...
            <?php if (!empty($trust_items)) : ?>
                <ul class="cal-cat-hero__trust">
                    <?php foreach ($trust_items as $item) :
                        $svg = function_exists('cal_get_icon') ? cal_get_icon($item['icon']) : '';
                        ?>
                        <li class="cal-cat-hero__trust-item">
                            <span class="cal-cat-hero__trust-icon" aria-hidden="true"><?php echo $svg; ?></span>
            ...
            <?php if ($intro !== '') : ?>
                <p class="cal-cat-hero__intro"><?php echo esc_html($intro); ?></p>
            <?php endif; ?>
            ...
            <?php if (!empty($filters) && is_array($filters)) : ?>
                <nav class="cal-cat-hero__filters" aria-label="<?php esc_attr_e('Quick filters', 'cyberatlaslab'); ?>">
                    <?php foreach ($filters as $row) :
                        ...
                        if ($anchor[0] !== '#') {
                            $anchor = '#' . $anchor; // 宽容处理：后台忘写 # 自动补
                        }
                        ?>
                        <a class="cal-cat-hero__filter" href="<?php echo esc_attr($anchor); ?>">
 ...
 * =============================================================================
 * 五个锚点与页面结构一一对应：#picks / #comparison / #methodology / #guide / #faq
 * 行为：CSS sticky 常驻桌面；IntersectionObserver 检测吸附态加阴影；scrollspy 高亮当前节
 * DOM 固定于 Hero 之后，永不移动（SEO/屏幕阅读器/锚点一致性三保障）
 * 用法：[cal_anchor_nav]
 * =============================================================================
 */
if (!function_exists('cal_anchor_nav_shortcode')) {
    ...
    function cal_anchor_nav_shortcode($atts)
    {
        ...
        ob_start();
        // 哨兵：sticky 状态检测锚点，无视觉
        echo '<span class="cal-anchor-sentinel" aria-hidden="true"></span>';
        ?>
        <nav class="cal-anchor-nav" aria-label="<?php esc_attr_e('Category sections', 'cyberatlaslab'); ?>">
            <ul class="cal-anchor-nav__list">
                <?php foreach ($items as $anchor => $label) : ?>
                    <li class="cal-anchor-nav__item">
                        <a class="cal-anchor-nav__link" href="#<?php echo esc_attr($anchor); ?>" data-anchor="<?php echo esc_attr($anchor); ?>">
                            <?php echo esc_html($label); ?>
                        </a>
                    </li>
                <?php endforeach; ?>
...
/**
 * =============================================================================
 * 40. 分类页测试方法折叠框 - [cal_how_test]
 * =============================================================================
 * 数据源：cat_how_test_title / cat_how_test_content（分类页数据组）
 * 形态：<details> 折叠卡，复用 FAQ 手风琴的独占折叠经验
 * 锚点：id="methodology"（Anchor Nav "How We Test" 跳转目标）
 * 尾部：若 Score Engine 方法论 URL 可用，自动挂 "Learn more" 链接
 * 空值纪律：标题或内容任一为空整块不渲染
 * 用法：[cal_how_test]
 * =============================================================================
 */
if (!function_exists('cal_how_test_shortcode')) {
    ...
    function cal_how_test_shortcode($atts)
    {
        ...
        $content = get_field('cat_how_test_content', $post_id);
        ...
        // 方法论页链接（Score Engine 已建）
        $methodology_url = function_exists('crs_methodology_url') ? crs_methodology_url() : '';

        ob_start();
        ?>
        <section class="cal-how-test" id="methodology">
            <details class="cal-how-test__box">
                <summary class="cal-how-test__toggle">
                    <span class="cal-how-test__toggle-text"><?php echo esc_html($title); ?></span>
                    <span class="cal-how-test__toggle-icon" aria-hidden="true"></span>
                </summary>
                <div class="cal-how-test__content">
                    <?php echo wp_kses_post($content); ?>
                    <?php if ($methodology_url !== '') : ?>
                        <p class="cal-how-test__more">
                            <a href="<?php echo esc_url($methodology_url); ?>">
                                <?php esc_html_e('Learn more about our testing methodology', 'cyberatlaslab'); ?>
                            </a>
                        </p>
                    <?php endif; ?>
                </div>
...
/**
 * =============================================================================
 * 41. 分类页对比因素六卡 - [cal_factors]
 * =============================================================================
 * 数据源：cat_factors repeater（factor_key + factor_text，固定 6 行）
 * 标题来源：get_field_object 读 select choices 的英文段（自动剥离中文括号），
 *           与后台选项单一来源，改选项全站同步
 * ⓘ 复用：factor_key 命中评分库的维度自动挂 tooltip，不命中静默跳过
 * 空值纪律：无数据整块不渲染；单行缺 text 跳过该行
 * 用法：[cal_factors]  /  [cal_factors title="How We Compare VPNs"]
 * =============================================================================
 */
if (!function_exists('cal_factors_shortcode')) {

    function cal_factors_shortcode($atts)
    {
        $atts = shortcode_atts(array(
            'title' => '',
        ), $atts, 'cal_factors');
        ...
        $rows = get_field('cat_factors', $post_id);
        if (empty($rows) || !is_array($rows)) {
            return '';
        }

        // select 选项表：key → 英文 label（剥括号）
        $choices = array();
        $field_obj = get_field_object('factor_key');
        if ($field_obj && !empty($field_obj['choices']) && is_array($field_obj['choices'])) {
            foreach ($field_obj['choices'] as $key => $label) {
                $choices[$key] = trim(preg_replace('/\s*[（(].*?[）)]\s*$/u', '', $label));
            }
        }

        $title = trim((string) $atts['title']);
        if ($title === '') {
            $title = __('How We Compare', 'cyberatlaslab');
        }

        ob_start();
        ?>
        <section class="cal-factors" id="factors" aria-labelledby="cal-factors-title-<?php echo esc_attr($post_id); ?>">
            <h2 class="cal-factors__title" id="cal-factors-title-<?php echo esc_attr($post_id); ?>">
                <?php echo esc_html($title); ?>
            </h2>
            <ul class="cal-factors__grid">
                <?php foreach ($rows as $row) :
                    $text = !empty($row['factor_text']) ? trim((string) $row['factor_text']) : '';
                    if ($text === '') continue;
                    $key = !empty($row['factor_key']) ? $row['factor_key'] : '';
                    $label = ($key !== '' && isset($choices[$key])) ? $choices[$key] : $key;
                    $tip = ($key !== '' && function_exists('crs_render_score_tooltip'))
                        ? crs_render_score_tooltip($key)
                        : '';
                    ?>
                    <li class="cal-factor">
                        <h3 class="cal-factor__name">
                            <?php echo esc_html($label); ?><?php echo $tip; ?>
                        </h3>
                        <p class="cal-factor__text"><?php echo esc_html($text); ?></p>
                    </li>
                <?php endforeach; ?>
            </ul>
        </section>
...
if (!shortcode_exists('cal_factors')) {
    add_shortcode('cal_factors', 'cal_factors_shortcode');
}


/**
 * 41.5 产品 Logo 渲染（上下文无关版）：SVG 内联优先，位图走 attachment image
